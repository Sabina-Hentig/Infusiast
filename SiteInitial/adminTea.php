...
<div class="container">
    ...
    <div class="forms-container">
        <h1>Add Teas</h1>
        <br><br>
        <form class="form-container" action="add_tea.php" method="post" enctype="multipart/form-data">
            <div>
            ...
            <div>
                ...
                <select id="category" name="category" required>
                    ...
                    $result = mysqli_query($cnx, "SELECT id, category_name FROM product_category_1");
            ...
            <div>
                ...
                <select id="type" name="type" required>
                    ...
                    $result = mysqli_query($cnx, "SELECT id, category_name FROM product_category_2");
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo "<option value='{$row['id']}'>{$row['category_name']}</option>";
                    }
                    mysqli_close($cnx);
            ...
            <br>
            <button class="btn" type="submit">Add Tea</button>
        </form>
    </div>
    <br><br>
    <div class="query-container">
        <h1>Teas - inventory</h1>
        <table>
            <thead>
            <tr>
                <th>Name</th>
            ...
            $query = "SELECT p.id, p.product_name, pi.quantity, p.is_active FROM product p JOIN product_inventory pi ON p.id = pi.product_id";
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<td>";
                echo "<button type='button' class='actions-btn' onclick=\"window.location.href='edit_tea.php?id=" . $row['id'] . "'\">Edit</button> ";
                echo "<button type='button' class='actions-btn' onclick=\"if(confirm('Are you sure you want to delete this item?')) { window.location.href='delete_tea.php?id=" . $row['id'] . "'; }\">Delete</button>";
                echo "</td>";
                echo "</tr>";
            }
            mysqli_close($cnx);
